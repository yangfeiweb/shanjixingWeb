...
export default {
  data() {
    return {
      ...
      chartData: {
        columns: ["日期", "学习时间(分钟)", "学习单词数(个)"],
        rows: []
      },
      chartSettings: {
        metrics: ["学习时间(分钟)", "学习单词数(个)"],
        dimension: ["日期"]
      }
    };
  },
  mounted() {
    ...
    this.monthArr = this.getCurrMonthDay(new Date());
    console.log("-----------当前月所有日期", this.monthArr);
    // 获取studentNo
  ...
  methods: {
            ...
            if (code === 200) {
              ...
              if (this.statisticsType === "BY_WEEK") {
                ...
                newList.forEach(e => {
                  ...
                  let learnTime=Math.ceil(e.time / (1000 * 60));
                  // console.log("----month", month);
                  // console.log("----day", day);
                  let showTime = month + "月" + day + "日";

                  let index = weekArr2.findIndex(a => a === e.day);
                ...
                console.log("-----week--rows", this.chartData.rows);
              } else if (this.statisticsType === "BY_MONTH") {
                let monthArr2 = [];
                this.monthArr.forEach(e => {
                  let n = e.replace(/-/g, "");
                  // console.log("------e", n);
                  monthArr2.push(n);
                  let month = n.slice(4, 6);
                ...
                console.log("-------monthArr2", monthArr2);
                ...
                newList.forEach(e => {
                  ...
                  let index = monthArr2.findIndex(a => a === e.day);
                  console.log("-----------------index", index);
                  this.chartData.rows[index] = {
                    日期: showTime,
                    '学习时间(分钟)': learnTime,
                ...
                console.log("-----month--rows", this.chartData.rows);
              } else if (this.statisticsType === "BY_YEAR") {
                this.yearArr.forEach(e => {
                  let month = e;
                ...
                newList.forEach(e => {
                  ...
                  let month = e.month.slice(4, 6);
                  let learnTime=Math.ceil(e.time / (1000 * 60));
                  // console.log("----year", year);
                  let showTime = year + "年" + month + "月";
                  let index = this.yearArr.findIndex(a => a === month);
                  console.log("-----------------index", index);
                  this.chartData.rows[index] = {
                    日期: showTime,
                    '学习时间(分钟)': learnTime,
                ...
                console.log("-----year--rows", this.chartData.rows);
              }
            }
          },
          () => {}
    ...
    getWeekTime(type) {
      ...
      if (type === "up") {
        ...
        this.dayValStart = this.weekArr[0];
        this.dayValEnd = this.weekArr[6];
        let nowWeek=this.getWeekSection(new Date()); 

        // let n=nowWeek[6].replace(/-/g,"");
        // let m=this.dayValEnd.replace(/-/g,"");
        // if(m===n){
        //   return
        // }else{
          this.chartData.rows = [];
