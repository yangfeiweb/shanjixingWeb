...
export default {
  mounted() {
    ...
    if (
      this.examType == "BOOK_WORD_BEFORE" ||
      this.examType == "BOOK_WORD_AFTER"
    ) {
      this.bookNo = this.$route.params.bookNo;
      this.size=20;
    } else if (
      this.examType == "COLUMN_WORD_BEFORE" ||
      this.examType == "COLUMN_WORD_AFTER"
    ) {
      this.bookNo = this.$route.params.bookNo;
  ...
  methods: {
    ...
    getData: function() {
      dataService
        .createParper(this.examType, this.size, this.bookNo, this.columnNo, this.grade)
        .then(
          res => {
      ...
      this.nextTimer = setTimeout(() => {
        ...
        if (this.curr + 1 >= this.wordList.length) {
          ...
          if (this.examType == "WORD") {
            let n = this.rightN / this.wordList.length; //this.wordList.length测试的题目数
            if (n <= 0.1) {
              this.testResult.score = Math.ceil(this.min / 3 * n)+"";
            } else if (n <= 0.9) {
              this.testResult.score = Math.ceil(this.min * n)+"";
            } else {
              this.testResult.score = Math.ceil(this.max * n)+"";
            }
          } else {
            let score = Math.floor(this.rightN/this.wordList.length*100);
