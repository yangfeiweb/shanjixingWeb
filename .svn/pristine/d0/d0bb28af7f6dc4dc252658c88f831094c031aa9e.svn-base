<template>
    <div class="learnIndex" v-loading='loadingShow' element-loading-spinner="el-icon-loading" element-loading-background="rgba(128,128,128, 0.8)">
        ...
        <div class="learnMain">
            ...
            <el-tabs v-show="trackListShow" class="elTabs" v-model="trackListValue" type="card" @tab-click="clickTab">
            ...
            <el-tabs v-show="!trackListShow" class="elTabs" v-model="trackListValue" type="card">
                <el-tab-pane key='-' value='-' name='-1' :label="trackListValue==='-1'?'-':learnArr[learnIndex].content1">
                </el-tab-pane>
            </el-tabs>
            <recognition v-show="learnType==='DEFAULT'||learnType==='LISTEN'" :audioImgUrl='audioImgUrl' :learnType='learnType' :imgShow='imgShow' :learnWordInfos='learnWordInfo' :btnData='btnData' :colorWord='colorWord'></recognition>
            <spelling :testType='learnType' v-show="learnType==='SPELL'" :learnWordInfos='learnWordInfo' :colorWord='colorWord'></spelling>
...
import recognition from "./children/recognition";
import spelling from "./children/spelling";
export default {
  components: {
    navHeader,
    learnFooter,
  ...
  data() {
    return {
      audioImgUrl: "../../../assets/images/listen.gif",
      learnType: "DEFAULT",
      examShow: false,
      imgShow: false,
      catalogList: [],
  ...
  mounted() {
    ...
    this.learnType = this.$route.params.learnType;
    if (this.bookInfo === undefined) {
      ...
      this.learnJsonData.learnType = this.learnType;
  ...
  methods: {
        ...
        if (this.siderData.selectPlayCount !== "") {
          ...
          if (this.siderData.needLearnColumnNo) {
            if (this.siderData.pause) {
              ...
              this.wordListDialogShow = false;
              this.preview();
              if (this.learnType === "LISTEN") {
                this.btnData.listenShow = false;
                this.btnData.listen1Show = false;
              }
            }
          }
        } else {
    ...
    playEnded(playNum) {
      ...
      if (this.learnType === "LISTEN") {
        if (!this.siderData.selectPlayCount) {
          if (this.btnData.answerBtnShow) {
            this.btnData.listen1Show = true;
            this.btnData.listenShow = false;
          }
        }
      }
    ...
    test() {
      ...
      if (this.promptContents === "learnOver") {
        if (this.learnType === "SPELL") {
          examType = "COLUMN_WORD_SPELL_AFTER";
        } else {
      ...
      if (this.promptContents === "chapterTest") {
        if (this.learnType === "SPELL") {
          examType = "COLUMN_WORD_SPELL_BEFORE";
        } else {
    ...
    clickWrong() {
      ...
      if (this.currentType === "learn") {
        ...
        let w = wrongArr.find(item => {
          return item.wordNo === wordNo;
        });
        // 如果当前单词在正确词表   将正确词数减一生词数加一   如果已经在错误词表的话  就不进行生词量的加
        if (o !== 0) {
        ...
        // 如果当前单词已经在当前栏目的错误词表
        let wordNotesWord = wrongArr.find(item => {
          return item.wordNo === wordNo;
        });
        // 如果当前单词已经点错过一次
        let wrongWord = this.worngWordData.find(item => {
          return item.wordNo === wordNo;
        });
        // 复习时当前单词在错误词表且没有点错过时   复习进度加一
        if (wordNotesWord && !wrongWord) {
      ...
      if (wrongArr.length === 0) {
        ...
        if (this.currentType === "learn") {
          ...
          this.getReviewNum(this.learnWordInfo);
        }
    ...
    correct() {
      ...
      if (this.learnType === "SPELL") {
        this.btnData.answerShow = true;
        this.btnData.spllingNextBtnShow = true;
      } else {
        this.btnData.successaAnswerShow = false;
        this.btnData.wrongBtnShow = false;
        this.btnData.answerBtnShow = true;
      ...
      if (this.currentTrackList === "Y") {
            ...
            for (let i = 0; i < l; i++) {
              if (wordNo === this.worngWordData[i].wordNo) {
                ...
                if (this.currentType === "learn") {
                  ...
                  this.learnJsonData.columns[0].learns.push(wordObject);
                  let wrongNotes = this.learnJsonData.columns[0].wordNotes;
                  let wordNotes = wrongNotes.find(item => {
                    return item.wordNo === wordNo;
                  });
                  this.$set(wordNotes, "reviewNum", 1);
                } else {
                  if (this.learnWordInfo.reviewNum === 0) {
                    let learnWordObject = {
                      wordNo: wordNo,
        ...
        if (this.learnIndex === this.learnArr.length - 1) {
          ...
          if (this.learnType === "SPELL") {
            // this.siderData.nextBtnShow = true;
          } else {
            this.learnIndex += 1;
            this.learnWordInfo = this.learnArr[this.learnIndex];
            this.btnData.listenShow = true;
    ...
    addLearnArr(n, rightCount) {
      ...
      if (this.wordState === "default") {
        this.learnArr.splice(this.learnIndex + n, 0, learnObject);
      } else {
    ...
    getAnswer() {
      ...
      this.btnData.answerBtnShow = false;
      this.btnData.answerShow = true;
      this.btnData.learnTypeShow = true;
      this.btnData.listenShow = false;
      this.btnData.listen1Show = false;
      this.imgShow = true;
      if (this.learnType === "DEFAULT") {
        this.$nextTick(() => {
    ...
    // 根据复习的次数，构建不同的对象
    getReviewNum(wordInfo) {
      let createTimes;
      let currentTimes = this.formatDateTime(new Date(), "ni");
      let columnNo = wordInfo.columnNo;
    ...
    getBookColumnLearnWords() {
      ...
      this.loadingShow = true;
      dataService
        .getBookColumnLearnWords(0, 10, this.learnType, this.columnNo)
        .then(
          res => {
            let result = res.data;
            let code = result.code;
    ...
    getBookReviewWords() {
      this.loadingShow = true;
      dataService.getBookReviewWords(0, 20, this.learnType, this.bookNo).then(
        res => {
          let result = res.data;
          let code = result.code;
          let data = result.data;
