<template>
    <div class="learnDialog">
        <el-dialog :close-on-click-modal='false' :title="warnDislog?titles:'学习提示'" :visible.sync="dialogShow" width="30%" @close='dialogHide' :show-close='false' center>
            <div v-if="reviewShow">
                <p>{{warnContent[0]}}</p>
                <p>{{warnContent[1]}}</p>
                <p>{{warnContent[2]}}</p>
            ...
            <span v-if="chapterTestShow||reviewShow" slot="footer" class="dialog-footer">
                ...
                <el-button @click="clickNo">否</el-button>
            </span>
            <span v-if="learnOverShow" slot="footer" class="dialog-footer bottonStyle">
                <el-button v-show="hanShow" @click="test">章节学后测试</el-button>
